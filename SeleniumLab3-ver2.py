...
login_check()
#get all products list with names, prices and buttons //For debug now

sum = 6 #value of all products in market
prod_mass = []
for i in range(0,sum):
    prod_locator = f"//a[@id='item_{str(i)}_title_link']"
    prod_mass.append(prod_locator)

###Welcome message
print("Welcome to our Swag Lab market")
print("Our market have next products:")
print("Choose one of the our product by number of product:")
print("Menu:")

s = 1
for d in prod_mass:
    prod_title = driver.find_element(By.XPATH, d)
    button_locator = f"//button[@id='add-to-cart-{prod_title.text.casefold().replace(' ', '-')}']"
    prod_bt = driver.find_element(By.XPATH, button_locator)
    print(f"{s} - {prod_title.text} ")
    s += 1
...
prod = driver.find_element(By.XPATH, f"//a[@id='item_{(int(choice) - 1)}_title_link']")
print("Product:", prod.text)
button_locator = f"//button[@id='add-to-cart-{prod.text.casefold().replace(' ', '-')}']"
add_cart1 = driver.find_element(By.XPATH, button_locator)
add_cart1.click()
print("Product has added into basket")
time.sleep(1)
prod1_etaloname = prod.text    #find and set as etalon
...
time.sleep(1)
price = driver.find_element(By.XPATH, "(//div[@class='inventory_item_price'])")
price_etalon = price.text


#Check that basket contains our products with prices
prod1_basket = driver.find_element(By.XPATH, f"//a[@id='item_{(int(choice) - 1)}_title_link']")
price1_basket = driver.find_element(By.XPATH, "(//div[@class='inventory_item_price'])")
assert prod1_basket.text == prod1_etaloname
assert price1_basket.text == price_etalon
...
prod1_overview = driver.find_element(By.XPATH, f"//a[@id='item_{(int(choice) - 1)}_title_link']")
price1_overview = driver.find_element(By.XPATH, "(//div[@class='inventory_item_price'])")
assert prod1_overview.text == prod1_etaloname
assert price1_overview.text == price_etalon
print("Product:", prod1_overview.text, "with price:", price1_overview.text, "is present into overview order")
