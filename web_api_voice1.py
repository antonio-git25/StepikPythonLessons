...
def practice_3_2_7():
    ...
    print("response.status_code: ", response.status_code)  # числовой код ответа сервера
    print("response.text: ", response.text)  # тело ответа
    print("response.headers: ", response.headers)  # словарь HTTP-заголовков
    print("response.cookies: ", response.cookies)  # объект cookies


def fake_user_agent():
    url = 'http://httpbin.org/user-agent'
    my_headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) NiggerWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36',
    }
...
def session_headers():
    ...
    # Заголовки по умолчанию для всей сессии
    session.headers.update({"User-Agent": "Mozilla/5.0 (X11; Linux x86_64)"})
    # Первый запрос наследует эти заголовки
    resp_default = session.get(url)
    # Напечатает Mozilla/5.0 (X11; Linux x86_64)
    print(resp_default.json()["headers"]["User-Agent"])

    # Разовая замена заголовка только для этого запроса
    resp_override = session.get(url, headers={"User-Agent": "CrawlerBot/2.0"})
    # Напечатает CrawlerBot/2.0
    print(resp_override.json()["headers"]["User-Agent"])
